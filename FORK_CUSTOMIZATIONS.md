...
| Canvas node's "+"/Split buttons sat right next to each tab's X close button, always visible, making it easy to misclick the wrong control | `DockTabStack.tsx`: first pass hid them behind `group-hover` on the whole tab-bar row, but hovering a TAB to click its own X also re-revealed them at the same spot — no actual fix. Corrected: the "+"/Split pair now has its own small wrapper with plain self `hover:opacity-100` (not `group-hover`), so only hovering that specific trailing corner reveals them — hovering a tab to close it never does | `(hover-reveal-split-btn)` |
