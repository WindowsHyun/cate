...
| Right-click "Open as Text" / "Open in New Canvas Node" on any file NOT at the workspace root silently fell back to plain "Open" (e.g. a nested `.html` file opened as a browser page instead of an editor) | `FileTreeNode.tsx`'s recursive `<FileTreeNode>` render for a folder's children never forwarded the `onFileOpenAsText`/`onFileOpenNew` props — only the top-level render (in `FileExplorer.tsx`) had them, so every nested file lost both actions | `(filetree-recursion-props)` |
| Markdown preview pane: text could not be drag-selected with the mouse | Canvas node chrome sets `userSelect: 'none'` on the whole node (title bar + body) to prevent accidental selection while dragging; Monaco's own styling overrides this for source mode, but the plain rendered-HTML markdown preview had no override. Added `userSelect: 'text'` on the preview container | `(preview-select)` |
| Opening the same local HTML file repeatedly kept spawning duplicate browser panels | `openFileGrouped`/`openFileAsPanel` now check for an existing browser panel with a tab already showing that `file://` URL (via `revealOnce`) before creating a new one, mirroring the editor/document/database dedup above | `(dedup-html-open)` |
| Extension-grouped file opens (Feature 13) silently stopped grouping/deduping once the target canvas node scrolled off-screen — reopening a file, or opening a new same-extension file, spawned a brand-new node instead of joining/reusing the existing one | Canvas nodes are viewport-culled (unmounted) when off-screen, which tears down their per-node DockStore registration. `findGroupNodeForExt`'s node scan and the new dedup's `revealOnce` both only checked the LIVE store and silently skipped unmounted nodes. Both now read via `getNodeDockLayout` (live store when mounted, falls back to the persisted `node.dockLayout` projection otherwise); grouping into an unmounted node writes the new tab directly into that projection via `setNodeDockLayout` since there's no live store to mutate | `(dedup-offscreen-node)` |
| Canvas node's "+"/Split buttons could sit right next to each tab's X close button with no guaranteed gap, making it easy to misclick the wrong control when the bar is full | `DockTabStack.tsx`: two earlier passes only changed *when* +/split are drawn (group-hover, then self-hover) — that changes timing, not the underlying layout, which had no `shrink-0` on the trailing +/split cluster or the trailing-controls (lock/opacity/close) group, so nothing guaranteed they'd hold their width instead of the tab row's `overflow-hidden` clipping into them. Fixed the actual layout: `shrink-0` on both trailing groups (only `DockTabBar`, already `flex-1 min-w-0`, gives up space) plus a real divider (`w-px h-5 bg-surface-5`, the same one `CanvasToolbar` uses between its own button groups) before each — a fixed floor instead of a hover trick | `(hover-reveal-split-btn)` |
| App wouldn't quit: with terminals running, the confirmation dialog appeared once, but confirming it then hung forever with no further dialog or response | Two disconnected quit-tracking systems: `windowFactory.ts`'s last-main-window `close` gate checks `isQuitCommitted()` (from `lifecycle/quitConfirm.ts`), but `index.ts`'s own inline `before-quit` handler tracked completion with a separate local `sessionFlushed` flag and never called `markQuitCommitted()`. So `isQuitCommitted()` stayed permanently false: once the session flush finished, `app.quit()` tried to close the window, the gate saw `isQuitCommitted() === false` and prevented it, re-triggering `app.quit()` — bouncing forever between "close prevented" and "before-quit already flushed, return immediately" with no dialog and no log line. Fixed by calling `markQuitCommitted()` at both points `index.ts` considers the flush done | `(quit-hang-uncommitted)` |
| Clicking a file already open as a background tab in a grouped canvas node brought the node into view but left whatever tab was already showing — the clicked file never actually appeared | `canvasBridge.ts`'s `focusPanelNode` only called `focusAndCenter(nodeId)`, which pans/centers the NODE but never touches which of its internal tabs is active. Now also switches the node's active tab to the clicked panel: `setActiveTab` on the live per-node DockStore when mounted, or writes `activeIndex` directly into the node's persisted `dockLayout` projection when off-screen (same live-or-persisted duality as the grouping fix above) | `(focus-node-active-tab)` |
| "New Workspace" silently did nothing once 10 workspaces already existed | `workspaceSlice.ts`'s `addWorkspace()` capped at 10 workspaces and, past the cap, returned the currently-selected id with no new workspace and no feedback of any kind — indistinguishable from a dead button. Session restore was never subject to this cap (it adds workspaces by explicit `id`, which skips the check), so a normal multi-repo user easily accumulates far more than 10 through ordinary use and then hits a button that appears broken. No test depended on the cap and nothing else enforces a 10-workspace invariant, so it wasn't protecting anything — removed | `(workspace-cap-removed)` |
| Moving a workspace to a group from the sidebar context menu produced zero visible change — the workspace stayed under its old group (or ungrouped) | `moveWorkspaceToGroup()` correctly updates `workspace.groupId` in the store, but `useWorkspaceList()`'s custom equality function (used by `ProjectList.tsx` to read the workspace array) never compared `groupId` — only id/name/color/rootPath/runtime fields. Zustand's `useStoreWithEqualityFn` treats "equal per this function" as "nothing changed, skip the re-render", so the grouped sidebar view never re-rendered no matter how many times `groupId` actually changed underneath it. Added `groupId` to the comparator | `(worklist-groupid-equality)` |
| "Another Cate instance has this project open" kept reappearing on every single restart, even with only one Cate ever running | `projectLock.ts`'s `acquireProjectLock` treated ANY live pid found in the stale `.cate/workspace.lock` as "still held" — but a quit (or crash) that leaves the lock file behind means that pid number can later be reassigned by the OS to a totally unrelated process (a system service, some other app), which then reads as "still alive" forever. Added `isLiveCateProcess`: checks the pid is actually running Cate or the unbranded dev Electron binary (`ps -o comm=` / `tasklist`, matched against `/cate\|electron/i`) before treating it as a real owner; an unverifiable pid now fails open (reclaims) instead of blocking, consistent with this lock already being advisory-only | `(project-lock-pid-reuse)` |
