...
| Canvas node's "+"/Split buttons sat right next to each tab's X close button, always visible, making it easy to misclick the wrong control | `DockTabStack.tsx`: added `group` to the tab-bar row, `+` button and Split button wrapper now `opacity-0 group-hover:opacity-100` (Split stays visible while its type-picker menu is open) — hidden at rest, revealed on hover, same convention as the tab X button | `(hover-reveal-split-btn)` |
