...
const strings = {
  ...
  'canvas.fitPanelsThreePanelLayout': { en: 'Odd panel count fit layout', ko: '홀수 패널 맞춤 레이아웃' },
  'canvas.fitPanelsThreePanelLayout.desc': { en: 'Layout for "Fit Panels to Screen" (⌘5) when an odd number of panels are open (e.g. 3 → 2/1, 5 → 3/2).', ko: '패널 개수가 홀수일 때 "패널을 화면에 맞추기"(⌘5)에 적용되는 레이아웃 (예: 3개 → 2/1, 5개 → 3/2).' },
  'canvas.fitPanelsThreePanelLayout.vertical': { en: 'Vertical (default)', ko: '세로 (기본값)' },
  'canvas.fitPanelsThreePanelLayout.horizontal': { en: 'Horizontal (2 rows, extra panel on top)', ko: '가로 (2줄, 위쪽에 여분 패널)' },
